...
<template>
  <div
    id="mobile-menu-id"
    class="absolute flex w-full items-center gap-17 bg-theme-white px-6 py-11 md:hidden"
  >
    <button
      type="button"
    ...
    <nav>
      <ul class="flex gap-8 font-semibold tracking-tighter text-theme-black">
        <li v-for="link in links" :key="link.title">
          <NuxtLink :to="link.url" @click="$emit('close')">
            {{ link.title }}
          </NuxtLink>
        </li>
      </ul>
